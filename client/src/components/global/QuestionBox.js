  ...
  return (
    <div className='questionBox'>
      ...
      <div className="questionBox__answer">
        {
          choice?.map((item, idx) => (
            <div key={`question${questionIdx}-choice${idx}`} className='inputGroup inputGroup--answer'>
              {
                answer
                ? (
                  <input type='radio' checked={Number(answer[questionIdx]) === idx ? true : false} disabled={true} />
                )
                : (
